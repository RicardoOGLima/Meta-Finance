...
import { Plus, PieChart as PieIcon, TrendingUp, Edit2, Trash2, ArrowRight, Star, Info, Filter } from 'lucide-react';
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 md:col-span-1">
          ...
          <h3 className="text-3xl font-bold mt-1">{formatCurrency(totalValue)}</h3>
              ...
              {classAllocation.map(c => (
                <div key={c.name} className="space-y-1.5 group">
                  <div className="flex justify-between items-end text-xs font-semibold">
                    <span className="text-slate-600 dark:text-slate-300 truncate max-w-[120px]">{c.name}</span>
                    <div className="flex gap-2 items-baseline">
                      <span className="text-slate-400 font-normal">Meta: {c.meta}%</span>
                      <span className="text-blue-600 font-bold text-sm">{c.value.toFixed(1)}%</span>
                    </div>
                  </div>
                  <div className="w-full bg-slate-100 dark:bg-slate-900 rounded-full h-2 relative group-hover:bg-slate-200 dark:group-hover:bg-slate-950 transition-colors">
                    {/* Discrete Target Goal Marker */}
                    {c.meta > 0 && (
                      <div 
                        className="absolute top-[-2px] bottom-[-2px] w-[3px] bg-slate-800 dark:bg-slate-400 z-10 rounded-full shadow-sm" 
                        style={{ left: `${c.meta}%` }}
                        title={`Alvo: ${c.meta}%`}
                      />
                    )}
        ...
        <div className="md:col-span-2 space-y-4">
          <div className="flex items-center gap-4 bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700">
            <div className="flex items-center gap-2 text-slate-400">
              <Filter size={18} />
              <span className="text-xs font-bold uppercase tracking-wider">Filtrar por Classe:</span>
            </div>
            <select 
              className="flex-1 md:flex-none px-4 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none text-sm transition-all focus:ring-2 focus:ring-blue-600"
              value={filterClass}
                    ...
                    <tr key={a.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          ...
                          <span className="text-[10px] text-slate-400 uppercase">{a.class}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-center">
                        <span className="px-2 py-1 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 rounded-lg font-bold flex items-center justify-center gap-1 w-fit mx-auto">
                          <Star size={12} className="fill-current" /> {a.score || 0}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-right font-mono">{formatCurrency(a.currentPrice)}</td>
                      <td className="px-6 py-4 text-right font-mono font-bold text-blue-600">{a.idealPercentage.toFixed(1)}%</td>
                      <td className="px-6 py-4 text-right font-mono">{a.currentPercentage.toFixed(1)}%</td>
                      <td className="px-6 py-4 text-center">
                        <span className={`px-2 py-1 rounded text-xs font-bold ${a.gap > 0 ? 'text-red-500 bg-red-50 dark:bg-red-900/20' : 'text-green-600 bg-green-50 dark:bg-green-900/20'}`}>
      ...
      {editingAsset && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-800 w-full max-w-md rounded-3xl shadow-2xl p-8 space-y-6 animate-in zoom-in-95 duration-200">
            <h3 className="text-xl font-bold">Editar {editingAsset.ticker}</h3>
            <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase mb-2">Preço Atual (R$)</label>
                <input 
                  type="number" 
                  className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none"
              ...
              <div className="grid grid-cols-1 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase mb-2">Nota (0-15)</label>
                  <input 
                  ...
                  <p className="mt-1 text-[10px] text-slate-400 italic flex items-center gap-1">
                    <Info size={10} /> O % Ideal é calculado automaticamente usando a Meta de Classe e esta Nota.
                  </p>
                </div>
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase mb-2">Observação</label>
                <textarea 
                  className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none resize-none min-h-[80px]"
            ...
            <div className="flex gap-3 pt-2">
              <button onClick={() => setEditingAsset(null)} className="flex-1 py-3 text-slate-500 font-bold hover:bg-slate-50 dark:hover:bg-slate-700 rounded-xl transition-all">Cancelar</button>
