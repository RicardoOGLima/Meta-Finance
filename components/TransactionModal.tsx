...
import { EXPENSE_CATEGORIES, INCOME_CATEGORIES, EXPENSE_SUBCATEGORIES, PAYMENT_METHODS } from '../constants';
...
const TransactionModal: React.FC<TransactionModalProps> = ({ type, isOpen, onClose, onSave }) => {
  const categories = type === 'RECCEITA' ? INCOME_CATEGORIES : EXPENSE_CATEGORIES;
  
  // Fix: Cast initial category to string to avoid strict enum inference that conflicts with string values from e.target.value.
  const [formData, setFormData] = useState({
    date: new Date().toISOString().split('T')[0],
    category: categories[0] as string,
    subcategory: type === 'RECCEITA' ? '' : EXPENSE_SUBCATEGORIES[0],
    description: '',
      ...
      <div className="bg-white dark:bg-slate-800 w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="px-8 py-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center">
          <h3 className="text-xl font-bold flex items-center gap-2">
            {type === 'RECCEITA' ? 'Nova Receita' : 'Nova Despesa'}
          </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {type === 'DESPESA' && (
              <div className="space-y-2">
                ...
                >
                  {EXPENSE_SUBCATEGORIES.map(sub => <option key={sub} value={sub}>{sub}</option>)}
                </select>
              </div>
            )}
